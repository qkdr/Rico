...
Made by Late
Follow-up: Added "Folders" view, folder cards, modal folder contents, and a simple protected-export helper (XOR-hex).
]]
...
function Library:CreateWindow(Settings: { Title: string, Size: UDim2, Transparency: number, MinimizeKeybind: Enum.KeyCode?, Blurring: boolean, Theme: string })
    ...
    --// Folders container (new)
    local FoldersContainer = Instance.new("Frame")
    FoldersContainer.Name = "FoldersContainer"
    FoldersContainer.Size = UDim2.new(1, 0, 1, 0)
    FoldersContainer.Position = UDim2.new(0, 0, 0, 0)
    FoldersContainer.BackgroundTransparency = 1
    FoldersContainer.Parent = Holder
    ...
    -- Add FoldersOnly toggle (hides sidebar and tabs)
    function Options:EnableFoldersOnly(Value)
        if Value then
    ...
    function Options:AddFolder(Settings)
        -- Settings: { Title = string, Image = string (rbxasset or url), Scripts = { {Name=, Callback=, LastChecked=} }, Order = number }
        local Folder = Instance.new("Frame")
        Folder.Name = Settings.Title or "Folder"
        Folder.Size = UDim2.new(0, 220, 0, 120)
        Folder.LayoutOrder = Settings.Order or 1
        Folder.BackgroundTransparency = 0
        Folder.BackgroundColor3 = Theme.Secondary
        Folder.BorderSizePixel = 0
        Folder.Parent = FoldersContainer
        ...
        TitleLabel.Parent = Info

        local CountLabel = Instance.new("TextLabel")
        CountLabel.Text = "عدد السكربتات: "..tostring(#(Settings.Scripts or {}))
        ...
        -- Slight transparency for the whole folder (as requested)
        Folder.BackgroundTransparency = 0.12

        -- Save in stored info
        StoredInfo["Folders"][Settings.Title] = { Folder = Folder, Scripts = Settings.Scripts }

        -- On click -> open folder modal with scripts list
        Connect(Folder.InputBegan, function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                -- create modal
                local Example = Clone(Examples["DropdownExample"]) or Instance.new("Frame")
                Example.Name = "FolderModal"
                Example.Position = UDim2.fromScale(0.5, 0.5)
                Example.AnchorPoint = Vector2.new(0.5, 0.5)
                Example.Size = UDim2.new(0, 420, 0, 300)
                Example.Parent = Window

                -- Title bar
                if Example:FindFirstChild("Top") and Example.Top:FindFirstChild("Title") then
                    Example.Top.Title.Text = Settings.Title
                end

                -- ensure ScrollingFrame exists
                local Scrolling = Example:FindFirstChildOfClass("ScrollingFrame") or Example:FindFirstChild("ScrollingFrame")
                if not Scrolling then
                    Scrolling = Instance.new("ScrollingFrame")
                    Scrolling.Size = UDim2.new(1, -20, 1, -60)
                    Scrolling.Position = UDim2.new(0, 10, 0, 40)
                    Scrolling.CanvasSize = UDim2.new(0, 0, 0, 0)
                    Scrolling.Parent = Example
                end

                -- fill scripts
                for i, scriptInfo in next, (Settings.Scripts or {}) do
                    local Btn = Clone(Components["Button"]) or Instance.new("TextButton")
                    if Btn:IsA("TextButton") then
                        local Title, Description = Btn:FindFirstChild("Title"), Btn:FindFirstChild("Description")
                        if Title and Title:IsA("TextLabel") then Title.Text = scriptInfo.Name or ("Script "..tostring(i)) end
                        if Description and Description:IsA("TextLabel") then Description.Text = scriptInfo.Description or "" end
                        ...
                        Connect(Btn.MouseButton1Click, function()
                            local ok, err = pcall(function()
                                if type(scriptInfo.Callback) == "function" then
                                    scriptInfo.Callback()
                                elseif type(scriptInfo.Callback) == "string" then
                                    -- if user passed source as string, try to run it
                                    local fn = loadstring(scriptInfo.Callback)
                                    if fn then fn() end
                                end
                            end)

                            if not ok then
                                warn("Folder script callback error:", err)
                            end
                            ...
                            task.wait(0.4)
                            pcall(function() Example:Destroy() end)
                        end)
                    else
                        -- fallback small button
                        local Fallback = Instance.new("TextButton")
                        Fallback.Text = scriptInfo.Name or ("Script "..tostring(i))
                        Fallback.Size = UDim2.new(1, -20, 0, 30)
                        Fallback.Parent = Scrolling
                        Connect(Fallback.MouseButton1Click, function()
    ...
    -- Simple protection helpers (do NOT remove important code) - returns XOR-hex encoded string
    local function xor_encode(s, key)
        key = key or "latekey"
        local out = {}
        for i = 1, #s do
            local b = s:byte(i)
            local k = key:byte(((i-1) % #key) + 1)
            out[i] = string.format("%02x", bit32.bxor(b, k))
        end
        return table.concat(out)
    end

    local function xor_decode(hex, key)
        key = key or "latekey"
        local bytes = {}
        for i = 1, #hex, 2 do
            local hx = hex:sub(i, i+1)
            local b = tonumber(hx, 16)
            local idx = ((i-1)/2) + 1
            local k = key:byte(((idx-1) % #key) + 1)
            bytes[#bytes+1] = string.char(bit32.bxor(b, k))
        end
        return table.concat(bytes)
    end

    -- public protect/export helper
    function Options:ProtectString(SourceString, Key)
        return xor_encode(SourceString, Key)
    end

    function Options:LoadProtected(HexString, Key)
        local ok, decoded = pcall(function() return xor_decode(HexString, Key) end)
        if not ok then return false, "decode_failed" end
        local ok2, res = pcall(function() return loadstring(decoded) end)
