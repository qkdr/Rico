--[[
	...
	Made by Late
	-- Extended with Folder support (by assistant)
]]
...
function Library:CreateWindow(Settings: { Title: string, Size: UDim2, Transparency: number, MinimizeKeybind: Enum.KeyCode?, Blurring: boolean, Theme: string })
	...
	-- Usage:
	-- local folder = Options:AddFolder({ Title="Folder", Description="...", Tab=someTab, Image="rbxassetid://...", LastChecked="..." })
	-- folder:AddScript({ Title="s1", Description="...", Callback=function() end, LastChecked="2025-12-17", Icon="rbxassetid://..." })
	-- ============================
	function Options:AddFolder(Settings)
		-- Settings: Title (string), Description (string), Tab (Instance), Image (string optional), Transparency (number optional), LastChecked (string optional)
		local TabParent = Settings.Tab or error("Folder must have a Tab (ScrollingFrame) as parent")
		local FolderTemplate = Components:FindFirstChild("Folder") -- optional template
		local Folder

		if FolderTemplate then
			Folder = Clone(FolderTemplate)
		else
			-- build folder UI from Button template for compatibility
			local Base = Clone(Components["Button"])
			Folder = Base
			-- add extra image and info labels
			local Img = Instance.new("ImageLabel")
			Img.Name = "FolderImage"
			Img.Size = UDim2.new(0, 48, 0, 48)
			Img.Position = UDim2.new(0, 6, 0, 6)
			Img.BackgroundTransparency = 1
			Img.Parent = Folder

			local TitleLabel = Folder:FindFirstChild("Labels") and Folder.Labels:FindFirstChild("Title") or Instance.new("TextLabel")
			-- reposition existing labels
			if TitleLabel and TitleLabel:IsA("TextLabel") then
				TitleLabel.Position = UDim2.new(0, 60, 0, 4)
				TitleLabel.TextXAlignment = Enum.TextXAlignment.Left
			end

			-- script count label
			local CountLabel = Instance.new("TextLabel")
			CountLabel.Name = "ScriptCount"
			CountLabel.Size = UDim2.new(0, 130, 0, 18)
			CountLabel.Position = UDim2.new(1, -140, 0, 6)
			CountLabel.BackgroundTransparency = 1
			CountLabel.Text = "عدد السكربتات: 0"
			CountLabel.Font = Enum.Font.SourceSans
			CountLabel.TextSize = 14
			CountLabel.TextColor3 = Theme.Description
			CountLabel.TextXAlignment = Enum.TextXAlignment.Right
			CountLabel.Parent = Folder

			local LastLabel = Instance.new("TextLabel")
			LastLabel.Name = "LastChecked"
			LastLabel.Size = UDim2.new(0, 130, 0, 14)
			LastLabel.Position = UDim2.new(1, -140, 0, 24)
			LastLabel.BackgroundTransparency = 1
			LastLabel.Text = "آخر فحص: " .. (Settings.LastChecked or "لم يتم الفحص")
			LastLabel.Font = Enum.Font.SourceSans
			LastLabel.TextSize = 12
			LastLabel.TextColor3 = Theme.Description
			LastLabel.TextXAlignment = Enum.TextXAlignment.Right
			LastLabel.Parent = Folder

			-- create content holder (collapsible)
			local Content = Instance.new("Frame")
			Content.Name = "FolderContent"
			Content.BackgroundTransparency = 1
			Content.Size = UDim2.new(1, 0, 0, 0)
			Content.Position = UDim2.new(0, 0, 0, Folder.Size.Y.Offset + 6)
			Content.ClipsDescendants = true
			Content.Visible = false
			Content.Parent = TabParent.Parent -- will reparent after insertion
			...
			Folder.FolderContent = Content
			Folder.ContentScroll = Scroller
			Folder.UIList = UIList
			Folder.ScriptCountLabel = CountLabel
			Folder.LastCheckedLabel = LastLabel
			Folder.FolderScripts = {}
		end

		-- set text and image
		local titleLabel = Folder:FindFirstChild("Labels") and Folder.Labels:FindFirstChild("Title")
		if titleLabel then
			titleLabel.Text = Settings.Title or "Folder"
		end

		local descLabel = Folder:FindFirstChild("Labels") and Folder.Labels:FindFirstChild("Description")
		if descLabel then
			descLabel.Text = Settings.Description or ""
		end

		local imageLabel = Folder:FindFirstChild("FolderImage")
		if imageLabel and Settings.Image then
		...
		-- visual transparency tuning to match library
		Folder.BackgroundTransparency = math.clamp(1 - (Setup.Transparency or 0), 0, 1)
		Folder.Parent = TabParent
		Folder.Visible = true

		-- collapse/expand behavior
		local Expanded = false
		local contentFrame = Folder:FindFirstChild("FolderContent")
		local scroller = Folder:FindFirstChild("ContentScroll")
		local list = Folder:FindFirstChildWhichIsA("UIListLayout", true) or Folder.UIList

		local function updateCountAndLast()
			local count = 0
			for _, v in pairs(Folder.FolderScripts or {}) do count = count + 1 end
			if Folder.ScriptCountLabel then
				Folder.ScriptCountLabel.Text = "عدد السكربتات: " .. tostring(count)
			end
			-- last checked label left as user-managed, or update based on latest item
		end
		...
		local function expand()
			...
			-- size calculation: sum children sizes
			task.wait()
			local total = 0
			...
			scroller.Size = UDim2.new(1, 0, 0, total)
			contentFrame.Size = UDim2.new(1, 0, 0, total)
			-- smooth show (fade/slide)
			Tween(scroller, 0.2, { Size = scroller.Size })
		end
		...
		local function collapse()
			...
			Expanded = false
			-- animate hide
			Tween(scroller, 0.2, { Size = UDim2.new(1, 0, 0, 0) })
			task.delay(0.18, function()
				contentFrame.Visible = false
			end)
		end

		Connect(Folder.MouseButton1Click, function()
			-- toggle expand
			if Expanded then
				collapse()
			else
		...
		-- provide method to add scripts into this folder
		local FolderObject = {}
		function FolderObject:AddScript(ItemSettings)
			-- ItemSettings: Title, Description, Callback, LastChecked, Icon
			local Item = Clone(Components["Button"])
			local TitleLabel, DescLabel = Options:GetLabels(Item)

			SetProperty(TitleLabel, { Text = ItemSettings.Title or "Script" })
			if DescLabel then
				SetProperty(DescLabel, { Text = ItemSettings.Description or "" })
			end

			-- small icon (if provided)
			if ItemSettings.Icon then
				local ico = Item:FindFirstChild("ICO")
				if ico and ico:IsA("ImageLabel") then
					SetProperty(ico, { Image = ItemSettings.Icon })
				else
					-- create a small icon
					local NewIco = Instance.new("ImageLabel")
					NewIco.Name = "ICO"
					NewIco.Size = UDim2.new(0, 28, 0, 28)
					NewIco.Position = UDim2.new(0, 6, 0, 6)
					NewIco.BackgroundTransparency = 1
					NewIco.Image = ItemSettings.Icon
					NewIco.Parent = Item
				end
			end
			...
			Connect(Item.MouseButton1Click, function()
				pcall(function() if ItemSettings.Callback then ItemSettings.Callback() end end)
				-- update folder last checked if provided
				if ItemSettings.LastChecked and Folder.LastCheckedLabel then
					Folder.LastCheckedLabel.Text = "آخر فحص: " .. tostring(ItemSettings.LastChecked)
				end
			end)

			Animations:Component(Item)
			SetProperty(Item, {
				Parent = scroller,
				Visible = true,
			...
			-- ensure scroller canvas size updated
			task.spawn(function()
				local total = 0
				for _, child in ipairs(scroller:GetChildren()) do
					if child:IsA("TextButton") or child:IsA("Frame") then
			...
			-- store
			table.insert(Folder.FolderScripts, Item)
			updateCountAndLast()
			return Item
		end

		-- helper to manually set last checked
		function FolderObject:SetLastChecked(text)
			if Folder.LastCheckedLabel then Folder.LastCheckedLabel.Text = "آخر فحص: " .. tostring(text) end
		end

		-- helper to get current count
		function FolderObject:GetCount()
			local count = 0
			for _, v in pairs(Folder.FolderScripts or {}) do count = count + 1 end
			return count
		end

		-- return the Folder frame and helpers
		FolderObject.Frame = Folder
		FolderObject.FolderFrame = contentFrame
		FolderObject.ContentScroller = scroller
